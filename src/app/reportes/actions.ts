...
export async function obtenerEstadisticasPorTipoEquipo(
  ...
  let query = supabase
    ...
    .select(`
      ...
      `)
    .eq('sucursales.empresa_id', empresaId); // Filtering via sucursales relation if possible or just company context

  // Mejorar consulta para filtrar por empresa a través de sucursal
  query = supabase
    .from('ordenes_servicio')
    .select(`
        tipo_equipo_id,
